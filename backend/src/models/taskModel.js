...
const taskSelect = `SELECT t.task_id, t.project_id, t.assigned_to AS assigned_to_id, t.assigned_by AS assigned_by_id,
  ...
  JOIN employees e1 ON t.assigned_to = e1.employee_id JOIN employees e2 ON t.assigned_by = e2.employee_id`;
...
const createTask = (project_id, assigned_to, assigned_by, task_title, description, priority, status, assigned_date, due_date) =>
  pool.query(`INSERT INTO tasks (project_id, assigned_to, assigned_by, task_title, description, priority, status, assigned_date, due_date)
    VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`, [project_id, assigned_to, assigned_by, task_title, description, priority, status, assigned_date, due_date]);

const updateTask = (id, project_id, assigned_to, task_title, description, priority, status, assigned_date, due_date) =>
  pool.query(`UPDATE tasks SET project_id=$1, assigned_to=$2, task_title=$3, description=$4, priority=$5, status=$6,
...
const updateTaskStatus = (id, status) => pool.query(`UPDATE tasks SET status=$1,
  completed_date=CASE WHEN $1 = 'Completed' THEN COALESCE(completed_date, CURRENT_DATE) ELSE NULL END WHERE task_id=$2 RETURNING *`, [status, id]);
